var datePicker = (function($) {
	...
	datePicker.prototype = {
		build: function() {
			this.wrapp = this.input.wrap('<div class="datepicker_w">');
			...
			} else this.date_max = ""; if (this.date_min != "" && this.date_min.match(this.reg)) {
				...
				if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() && this.currentMonth.getMonth() == newMonth.getMonth())) {
					...
					for (var i = 0; i <= numDays; i++) {
						...
						if (this.isFirstDayOfWeek(currentDay)) {
							...
							else dayCells += "<tr>"; if (this.show_week == 1) dayCells += '<td class="week_num">' + this.getWeekNum(currentDay) + '</td>'
						}
		...
		selectDate: function(date) {
			...
			if (this.select_week == 1 && !this.isFirstDayOfWeek(date)) date = new Date(date.getFullYear(), date.getMonth(), (date.getDate() - date.getDay() + this.start_of_week), 12, 00);
		...
		setPosition: function() {
			var offset = this.offset || this.input.offset();
			this.rootLayers.css({
